...
export function Select({
	...
	return (
		...
		>
			<button
				className='btn btn-link bg-bg2 w-full py-2 px-4'
				onClick={(e) => {
					e.stopPropagation()
			...
			>
				<div className='overflow-hiddenmax-h-100 overflow-y-auto border border-black' data-lenis-prevent>
					<ul className='flex-col bg-bg2  p-0.5'>
						{options?.map((id) => (
							<li key={id}>
								<button onClick={(e) => onSelectCurrent(e, id)} className='btn link w-full *:text-left'>
