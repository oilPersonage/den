    ...
    >
      ...
      <div
        className={`grid absolute top-0
					${isAutoWidth ? "" : "right-0"} left-0 ${!isOpened ? "grid-rows-[0fr] z-1" : "grid-rows-[1fr] z-2 min-w-max"}`}
      >
        <div className="overflow-hidden overflow-y-auto" data-lenis-prevent>
          <ul className="flex-col max-h-100 min-h-fit bg-bg2 p-0.5 border border-black">
            {options?.map((el) => (
              <li
                key={el.value}
                data-value={el.value}
                onClick={(e) => onSelectCurrent(e, el.value)}
              >
                {children ? (
                  children(el)
                ) : (
