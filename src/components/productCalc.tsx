...
export default function ProductCalc({ price, name }: { price: number; name: string }) {
	...
	return (
		<div className='flex flex-col gap-md mb-lg max-md:order-4 max-md:mt-lg'>
			<div className='flex gap-md relative z-10 max-md:mb-sm'>
					...
					<div className='product-count'>
						...
						>
							<svg className='w-3' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
								<path d='M24 10.5V13.5L0 13.5L2.62268e-07 10.5L24 10.5Z' fill='black' />
							</svg>
			...
			<div className='flex gap-sm md:gap-md max-md:flex-col md:items-center'>
				<a
					href='#'
				...
				<div className='flex flex-col' data-ai='4'>
					<div className='flex flex-wrap gap-x-sm text-xl max-md:*:text-2xl max-md:mb-sm'>
						<p>от {priceFormatter(price)} ₽</p>
						<p>x {priceFormatter(count)}</p>
						<p>=</p>
						<p>от {priceFormatter(price * count)} ₽</p>
						{!isSelfDeliver ? <p className='text-black/40 !text-p'>+ Стомость доставки</p> : null}
					</div>
					<p className='text-black/40'>Стоимость доставки: {isSelfDeliver ? 'Бесплатно' : 'Зависит от региона'}</p>
				</div>
			</div>

			<Request count={count} name={name} delivery={delivery} paymentMethod={paymentMethod} price={totalPrice} />
		</div>
	)
}
