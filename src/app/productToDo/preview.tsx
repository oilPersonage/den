...
export default function Preview({ product, isOpened, setIsOpened }) {
	const { h1, headingDescription, characters, price, name, pictures = [], tag } = product
	...
	return (
		<div className={`fixed bg-bg overflow-y-auto inset-0 ${isOpened ? 'z-10' : '-z-1'}`}>
			<button className='absolute right-4 top-4 btn link black' onClick={() => setIsOpened(false)}>
				Закрыть
				<span></span>
				<span></span>
				<span></span>
