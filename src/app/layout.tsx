...
export default async function RootLayout({
  ...
  viewport-fit=cover, shrink-to-fit=no'
				...
				<link rel='icon' type='image/svg+xml' href={`${process.env.NEXT_PUBLIC_BASE_PATH}/vite.svg`} />
			...
			window.scrollTo(0, 0)
				...
				<meta property='og:title' content='Строительная компания [Название] | Дома под ключ' />
				<meta
				...
				<meta name='twitter:description' content='Дома под ключ от производителя' />

				<meta name='business:contact_data:country_name' content='Россия' />
				<meta name='business:contact_data:city' content='[Город]' />
			</head>
			<body>
				<div id='dots-canvas' className='fixed inset-0 -z-1 pointer-events-auto select-none'></div>
			...
			<Script src={`${process.env.NEXT_PUBLIC_BASE_PATH}/scripts/main.js`} type='module' />
		</html>
	)
}
