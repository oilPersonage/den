...
export default function ProductsRent({ products }: { products: Product[] }) {
  ...
  return (
    <>
      <ul className="grid md:grid-cols-2 mt-lg gap-xl">
        {products.map(
          ({ slug, pictures, name, headingDescription, characters, rent }) => (
            <li key={slug}>
              ...
              <div className="flex max-md:flex-row-reverse md:items-center gap-md">
                <a
                  data-modal="request"
                  href="#"
                  className="btn black"
                  onClick={() => setRequest({ name, rent })}
                >
                  Оставить заявку
                  <span></span>
                  <span></span>
