...
export default function ProductCalc({ slug, product }: { slug: string; product: Product }) {
	...
	return (
		<div className='calc-section min-h-screen flex flex-col pt-header max-md:hidden'>
			<div className='flex flex-1'>
				<div className='flex flex-col gap-md py-md pl-xl pr-lg calc-left flex-1'>
					<div className='h1Box'>
					...
					<div className='flex items-start gap-lg relative z-4'>
						<div data-ai='4' className='flex flex-col gap-md'>
							...
							<div>
								...
								<Select size='sm' options={baseMaterials} value={material} onSelectAction={(v) => setMaterial(v)} />
							</div>
						</div>
						<div data-ai='4' className='flex min-w-35 max-w-35 min-h-35 bg-btn-hover'>
							<CustomImage
					...
					<div data-ai='4' className='flex flex-col gap-xs relative z-2'>
						<div className='flex flex-col mb-xs min-h-[34px] relative z-2'>
							...
							{containers.length < 5 && (
								<Select
									...
									customValue={(el) => (!el ? '+ Добавить' : <p className='text-start mb-0'>{el.data.name}</p>)}
								>
									{(el) => (
						...
						<ul className='mt-xs'>
							{containers.map((el, idx) => (
								<li className='flex items-center gap-sm text-nowrap' key={el.id}>
									...
									<div className='calc-block-actions ml-auto'>
										{idx !== 0 && (
											<button className='btn link' onClick={() => onChangePosition(idx, idx - 1)}>
					...
					<ProductPrice name={product.name} price={product.price} />
				</div>
				<div className='p-lg pt-md pb-xl calc-right bg-bg2 flex-1 min-w-295 max-w-295'>
					<div className='flex gap-lg mb-xl' data-ai='4'>
						<div className='grid grid-cols-2 ml-6 gap-xs gap-x-lg min-w-100'>
							...
							<div className='flex gap-sm items-center'>
								<CrossSvg className='bg-neutral-200 w-6 justify-center h-6! p-1! *:h-3' onClick={() => null} />
						...
						<div className='flex max-w-100' data-ai='4'>
							<p>Для изменения положения двери, окна или перегородки, нажмите и перетащите</p>
						</div>
					</div>
					<ul id='calc-wrapper' data-ai='4'>
						{containers.map((el, idx) => (
							<CalcBase
								isFirst={idx === 0}
								name={`${idx + 1} Блок`}
								...
								width={w}
								container={el}
								onChangeConainer={(v) => setContainers(containers.map((el) => (el.id === v.id ? v : el)))}
