...
const findMagnetPoint = (value: number, points: number[], threshold: number): { isSticky: boolean; value: number } => {
...
export function watchDrag(selector: string, parentId: string, parentSizes: [number, number]) {
	...
	function updateData(initialData: Item, rotated: boolean) {
		const { cX, cY, fixedXFn, fixedYFn, magX, magY, h, w, cropX, cropY } = calculateSizes(
			initialData.dom,
	...
	draggables.forEach((item) => {
		...
		if (isWall) {
			const cleanup = draggableHeight(initialData, updateData, [maxHeight, maxWidth])
		...
		const onMouseDown = (e: MouseEvent) => {
			...
			if (initialData.type !== 'wall') {
				...
				initialData.magnetPointsX = [initialData.magnetPointsX[0], ...moreMagnetX, initialData.magnetPointsX[1]]
				initialData.magnetPointsY = [initialData.magnetPointsY[0], ...moreMagnetY, initialData.magnetPointsY[1]]
			}
		...
		const onMouseMove = (e: MouseEvent) => {
			...
			initialData.finalX = initialData.rotated
				? valueX
				: findMagnetPoint(valueX, initialData.magnetPointsX, MAGNET_THRESHOLD).value
			const valueY = initialData.initDragY + dy
			initialData.finalY = initialData.rotated
				? findMagnetPoint(valueY, initialData.magnetPointsY, MAGNET_THRESHOLD).value
				: valueY
		...
		const onMouseUp = () => {
			...
			if (initialData.type === 'wall') {
				initialData.finalX = utils.snap(4)(initialData.finalX)
			}
			initialData.dragging = false
			parent.classList.remove('draggable')

			const stickyX = findMagnetPoint(initialData.finalX, initialData.magnetPointsX, MAGNET_THRESHOLD).isSticky
			const stickyY = findMagnetPoint(initialData.finalY, initialData.magnetPointsY, MAGNET_THRESHOLD).isSticky
			item.setAttribute('data-sticky', stickyX ? 'x' : stickyY ? 'y' : undefined)
