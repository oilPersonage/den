...
const itemsProgressText = [...document.querySelectorAll('.adv-right-progress p')] as HTMLElement[]
...
const fakeData = { p: 0 }
const arrayWidthsOurText = itemsOur.reduce((acc, el, index) => [...acc, acc[acc.length - 1] + el.clientWidth + 48], [0])
...
tl.onRender = (self) => {
	advLineAnimate.seek((self.progress / (advLineAnimate.duration / TL_DURATION)) * advLineAnimate.duration)
